...
public class Backup {
    ...
    // before 1.4.3
    private static final String SUFFIX_ONLYX = "onlyx";

    // cfbf = OnlyX Favorite Backup File
    private static final String SUFFIX_CFBF = "cfbf";

    // ctbf = OnlyX Tag Backup File
    private static final String SUFFIX_CTBF = "ctbf";

    // csbf = OnlyX Settings Backup File
    private static final String SUFFIX_CSBF = "csbf";

    private static final String JSON_ONLYX_KEY_COMIC_SOURCE = "s";
    private static final String JSON_ONLYX_KEY_COMIC_CID = "i";
    private static final String JSON_ONLYX_KEY_COMIC_TITLE = "t";
    private static final String JSON_ONLYX_KEY_COMIC_COVER = "c";
    private static final String JSON_ONLYX_KEY_COMIC_UPDATE = "u";
    private static final String JSON_ONLYX_KEY_COMIC_FINISH = "f";
    private static final String JSON_ONLYX_KEY_COMIC_LAST = "l";
    private static final String JSON_ONLYX_KEY_COMIC_PAGE = "p";
    ...
    public static Observable<String[]> loadFavorite(DocumentFile root) {
        return load(root, SUFFIX_ONLYX, SUFFIX_CFBF);
    }
...
*/
    ...
    public static Observable<List<Pair<Tag, List<Comic>>>> restoreTag(final ContentResolver resolver, final DocumentFile root, final String filename) {
        return Observable.create((Observable.OnSubscribe<List<Pair<Tag, List<Comic>>>>) subscriber -> {
    ...
    public static Observable<List<Comic>> restoreComic(final ContentResolver resolver, final DocumentFile root, final String filename) {
        return Observable.create((Observable.OnSubscribe<List<Comic>>) subscriber -> {
            ...
            String jsonString = readBackupFile(resolver, root, filename);
            try {
                if (filename.endsWith(SUFFIX_ONLYX)) {
                    list.addAll(loadComicArray(new JSONArray(jsonString), SUFFIX_ONLYX));
    ...
    public static Observable<Map<String, ?>> restoreSetting(final ContentResolver resolver, final DocumentFile root, final String filename) {
        return Observable.create((Observable.OnSubscribe<Map<String, ?>>) subscriber -> {
    ...
    private static List<Comic> loadComicArray(JSONArray array, String suffix) throws JSONException {
        ...
        switch (suffix) {
            case SUFFIX_ONLYX:
                for (int i = 0; i != array.length(); ++i) {
                    JSONObject object = array.getJSONObject(i);
                    int source = object.getInt(JSON_ONLYX_KEY_COMIC_SOURCE);
                    String cid = object.getString(JSON_ONLYX_KEY_COMIC_CID);
                    String title = object.getString(JSON_ONLYX_KEY_COMIC_TITLE);
                    String cover = object.getString(JSON_ONLYX_KEY_COMIC_COVER);
                    String update = object.optString(JSON_ONLYX_KEY_COMIC_UPDATE, null);
                    Boolean finish = object.has(JSON_ONLYX_KEY_COMIC_FINISH) ?
                            object.getBoolean(JSON_ONLYX_KEY_COMIC_FINISH) : null;
                    String last = object.optString(JSON_ONLYX_KEY_COMIC_LAST, null);
                    Integer page = object.has(JSON_ONLYX_KEY_COMIC_PAGE) ?
                            object.getInt(JSON_ONLYX_KEY_COMIC_PAGE) : null;
